...
# ─────────────────────────────────────────────
# MONTH / YEAR SELECTION (SIDEBAR TOP)
# ─────────────────────────────────────────────
...
with st.sidebar:
    ...
    sel_month = col_m.selectbox("Month", list(range(1, 13)),
                                ...
                                key='month_sel')
    sel_year = col_y.number_input("Year", min_value=2000, max_value=2100,
                                  value=st.session_state.sel_year, step=1,
                                  key='year_sel')
...
def get_month_info(year, month):
    """Return all dynamic date info for the selected month/year."""
    num_days = calendar.monthrange(year, month)[1]
    day_abbr = {d: dt.date(year, month, d).strftime('%a') for d in range(1, num_days + 1)}
    ...
    start_label = f"{calendar.month_abbr[month]} 01 {year}"
    end_label   = f"{calendar.month_abbr[month]} {num_days:02d} {year}"
    date_range_label = f"{start_label}  To  {end_label}"
    return num_days, day_abbr, sundays, saturdays, month_label, date_range_label

YEAR  = st.session_state.sel_year
MONTH = st.session_state.sel_month
NUM_DAYS, DAY_ABBR, SUNDAYS, SATURDAYS, MONTH_LABEL, DATE_RANGE_LABEL = get_month_info(YEAR, MONTH)
            ...
            if str(df.iloc[i, 0]).strip() == 'Emp. Code:':
                ...
                for j in range(1, len(df.iloc[i])):
                    ...
                    if not pd.isna(v) and str(v).strip() not in ['', 'nan', 'Emp. Name:']:
                        try:
                            emp_code = int(float(str(v)))
                            break
                        except:
                            pass
                if emp_code is None or i + 3 >= len(df):
                    i += 1
                    continue
...
def run_data_checks(emp_df, attendance):
    ...
    for _, emp in emp_df.iterrows():
        ...
        miss_days = [d for d in range(1, NUM_DAYS + 1) if
        ...
        if miss_days:
            ...
            issues.append({'severity': '🟠 MISS', 'category': 'Missing Punch',
                           'emp_code': code, 'emp_name': name,
                           'detail': f"{name} ({code}) missing punch on: {', '.join(f'{mon_abbr} {d}' for d in miss_days)}. Hours not counted."})
    ...
    for _, emp in emp_df.iterrows():
        ...
        for d in range(1, NUM_DAYS + 1):
            ...
            mon_abbr = calendar.month_abbr[MONTH]
            if in_t and t2m(in_t) > 14 * 60:
                suspect.append(f"{mon_abbr} {d} In={in_t.strftime('%H:%M')}")
            if out_t and t2m(out_t) < 10 * 60:
                suspect.append(f"{mon_abbr} {d} Out={out_t.strftime('%H:%M')}")
        if suspect:
...
def build_excel(emp_df, attendance, gate_passes=None):
    ...
    SHIFTIN_COL = 2; SHIFTOUT_COL = 3; DAY1_COL = 4
    DAY_END = DAY1_COL + NUM_DAYS - 1
    SUM1_COL = DAY_END + 1
    ...
    for company in emp_df['Company'].unique():
        ...
        for r, txt, sz in [(1, "Monthly Status Report (Detailed Work Duration)", 14),
                           (2, DATE_RANGE_LABEL, 10)]:
            ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=LAST_COL)
            c = ws.cell(r, 1, txt)
            c.font = fn(True, sz, 'FFFFFF'); c.fill = fl('1A3C6E'); c.alignment = al()
        ...
        for col, txt2, fg, bg in [(1, 'Days', '000000', 'EEF4FA'), (2, 'Sh.In', '1565C0', 'DDEEFF'),
                                   (3, 'Sh.Out', '1565C0', 'DDEEFF')]:
        ...
        for dept in c_emps['Department'].unique():
            ...
            for _, emp in d_emps.iterrows():
                ...
                for rr, h in [(HR, 18), (RS, 14), (RI, 14), (RO, 14), (RD, 14),
                               (RL, 14), (RE, 14), (ROT, 14), (RDP, 14), (RSP, 3)]:
                ...
                for d in range(1, NUM_DAYS + 1):
                    ...
                    if is_open:
                        raw_dur = (f'MAX(0,MIN({or_},TIME(16,0,0))-{ir})' if sun else f'MAX(0,{or_}-{ir})')
                    else:
        ...
        ws.column_dimensions['C'].width = 5.5
        for d in range(1, NUM_DAYS + 1):
            ws.column_dimensions[get_column_letter(DAY1_COL + d - 1)].width = 6.2
...
def build_timing_sheet(emp_df, attendance, company_filter=None, dept_filter=None):
    ...
    for company in filtered['Company'].unique():
        ...
        c = ws.cell(1, 1, f"Timing Sheet — {company} — {MONTH_LABEL}")
        ...
        for _, emp in c_emps.iterrows():
            ...
            for d in range(1, NUM_DAYS + 1):
                ...
                vals = [code, name, dept, shift_lbl, f"{mon_abbr} {d:02d}", DAY_ABBR[d],
...
with st.sidebar:
        ...
        if st.button("⚙️ Process Files", use_container_width=True, type='primary'):
            with st.spinner("Loading & validating..."):
                try:
                    ...
                    st.session_state.loaded_period = (MONTH, YEAR)
    ...
    if st.session_state.emp_df is not None:
        df = st.session_state.emp_df; issues_sb = st.session_state.issues or []
        st.markdown(f"**Period:** {MONTH_LABEL}")
        st.markdown(f"**Employees:** {len(df)}")
        st.markdown(f"**Companies:** {df['Company'].nunique()}")
        st.markdown(f"**Departments:** {df['Department'].nunique()}")
        cc2 = sum(1 for i in issues_sb if '🔴' in i['severity'])
        if cc2: st.error(f"⚠️ {cc2} critical issues need attention!")

# ─────────────────────────────────────────────
# MAIN HEADER  ← NOW DYNAMIC
# ─────────────────────────────────────────────
st.markdown(
    f'<div class="main-header">'
    f'<h1>📋 Monthly Attendance Report System</h1>'
    f'<p>{MONTH_LABEL} &mdash; Formula-Enabled Excel Generator</p>'
    f'</div>',
    unsafe_allow_html=True
)

if st.session_state.emp_df is None:
    st.info("👈 Select the month/year and upload files in the sidebar to get started.")
    with st.expander("📖 How to use"):
        st.markdown("""
        1. **Select Month & Year** at the top of the sidebar
        2. Upload **Employee Master** and **Attendance XLS**
        3. Click **Process Files**
        4. **⚠️ Check Issues tab first** — fix critical problems before using salary data
        5. Use **Open Shift Override** for employees with irregular hours
        6. Fix MISS punches, add Gate Passes, then download reports
        """)
    st.stop()
...
# ═══════════════════════════════════════════════
with tab1:
    st.subheader(f"📊 Attendance Summary — {MONTH_LABEL}")
    if critical_cnt:
    ...
    with col1:
        ...
        fig = px.bar(comp_data.melt(id_vars='Company', var_name='Status', value_name='Days'),
                     ...
                     title=f"Attendance by Company — {MONTH_LABEL}")
...
with tab2:
    ...
    if orphan_codes:
        ...
        st.dataframe(pd.DataFrame([{'Employee Code': c, 'Action': 'Add to Employee Master Excel'}
                                    for c in sorted(orphan_codes)]),
                     use_container_width=True, hide_index=True)
...
# ═══════════════════════════════════════════════
with tab3:
    st.subheader(f"👁️ View Attendance Report — {MONTH_LABEL}")
    ...
    for _, emp in d_emps.iterrows():
        ...
        with st.expander(f"{'🔓' if is_open else '🔒'} {code} : {emp['EmpName']}  |  {shift_lbl}"):
            ...
            view_df = pd.DataFrame.from_dict(rows_v, orient='index',
                                             columns=[f"{d} {DAY_ABBR[d][:2]}" for d in range(1, NUM_DAYS + 1)])
            ...
            st.dataframe(style_fn(color_status, subset=pd.IndexSlice[['Status'], :]),
...
with tab5:
    ...
    st.markdown("""
    Use this for employees who work **irregular hours** (e.g. split shifts).
    ...
    **Open Shift calculates:**
    ...
    - ✅ **No Late / Early** deductions

    > 💡 Overrides apply to this session only. To make permanent, update Employee Master Excel.
    """)
...
with tab6:
    ...
    with st.container(border=True):
        ...
        sel_code = emp_options[sel_lbl]
        sel_name = sel_lbl.split('—')[1].split('(')[0].strip()
        gp_day = gb.selectbox("📅 Date", list(range(1, NUM_DAYS + 1)),
                              format_func=lambda d: f"{mon_abbr} {d:02d} ({DAY_ABBR[d]})", key="gp_day")
        ...
        if st.button("➕ Add Gate Pass", type="primary", use_container_width=True):
            ...
            if not ot or not it: st.error("❌ Enter valid Out and In times (HH:MM)")
                ...
                st.session_state.gate_passes.append(
                    {'code': sel_code, 'name': sel_name, 'day': gp_day,
                     'date_str': f"{mon_abbr} {gp_day:02d} ({DAY_ABBR[gp_day]})",
                     'gp_out': ot.strftime('%H:%M'), 'gp_in': it.strftime('%H:%M'),
                     'duration_mins': dur, 'duration_str': m2hm(dur),
    ...
    st.markdown("---")
    if not gate_passes:
        st.info("📭 No gate passes recorded yet.")
    else:
        ...
        fd = gf2.selectbox("Filter Date", ['All'] + [f"{mon_abbr} {d:02d} ({DAY_ABBR[d]})"
                                                      for d in range(1, NUM_DAYS + 1)], key="gp_fd")
        ...
        st.markdown("### 📊 Summary")
        st.dataframe(pd.DataFrame([{'Code': c, 'Employee': n, 'Passes': v['count'],
                                     'Days': len(v['days']), 'Total Out': m2hm(v['total_mins'])}
                                    for (c, n), v in sorted(gps.items())]),
                     use_container_width=True, hide_index=True)
        st.markdown("---")
        d1, d2 = st.columns([3, 1])
        dopts = {f"#{i}|{g['name']}|{g['date_str']}|{g['gp_out']}–{g['gp_in']}": i
                 for i, g in enumerate(gate_passes)}
        if dopts:
        ...
        cb = io.StringIO()
        pd.DataFrame([{'Code': g['code'], 'Employee': g['name'], 'Date': g['date_str'],
                        'Out': g['gp_out'], 'In': g['gp_in'], 'Duration': g['duration_str'],
                        'Reason': g['reason'], 'Approved': g['approved_by']}
                       for g in gate_passes]).to_csv(cb, index=False)
        st.download_button("📥 Export Gate Passes CSV", data=cb.getvalue(),
                           file_name=f"GatePasses_{MONTH_LABEL.replace(' ','_')}.csv", mime="text/csv")
...
with tab7:
    ...
    if not issues:
        ...
        st.download_button("📥 Export Issues CSV", data=cb2.getvalue(),
                           file_name=f"DataIssues_{MONTH_LABEL.replace(' ','_')}.csv", mime="text/csv")

# ═══════════════════════════════════════════════
# TAB 8 — DOWNLOAD
# ═══════════════════════════════════════════════
with tab8:
    st.subheader(f"⬇️ Download Reports — {MONTH_LABEL}")
    if critical_cnt:
    ...
    with st.container(border=True):
        ...
        st.markdown(f"""
        - ✅ Period: **{MONTH_LABEL}** ({NUM_DAYS} days)
        ...
        if st.button("🔨 Generate Attendance Excel", type="primary", use_container_width=True, key='gen_att'):
            with st.spinner("Building Excel..."):
                try:
                    fname = f"Attendance_{MONTH_LABEL.replace(' ','_')}_Report.xlsx"
    ...
    with st.container(border=True):
        ...
        if st.button("🖨️ Generate Timing Sheet", type="primary", use_container_width=True, key='gen_ts'):
            with st.spinner("Building timing sheet..."):
                try:
                    fname2 = f"TimingSheet_{MONTH_LABEL.replace(' ','_')}.xlsx"
                    buf2 = build_timing_sheet(emp_df, att_working,
                                              None if ts_co == 'All' else ts_co,
                                              None if ts_de == 'All' else ts_de)
                    st.download_button(f"📥 Download {fname2}", data=buf2, file_name=fname2,
                                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
